`timescale 1ns/1ps
// ===========================================================================
// Testbench: two_ff_sync  --  single-bit level synchronizer.
// Drives a level from the "source" and checks it appears in the destination
// exactly STAGES destination-clocks later (the synchronizer latency).
// ===========================================================================
module tb_two_ff_sync;
  localparam STAGES = 2;
  ...
  initial begin
    $dumpfile("sim/two_ff_sync.vcd"); $dumpvars(0, tb_two_ff_sync);

    phase = "RESET"; dst_rst_n = 1'b0; din = 1'b0;
    repeat (2) @(negedge dst_clk); dst_rst_n = 1'b1;
    check(dout === 1'b0, "low after reset");

    // rising level -> propagates after STAGES clocks
    phase = "RISE"; @(negedge dst_clk); din = 1'b1;
    for (s = 0; s < STAGES; s = s + 1) begin
      @(posedge dst_clk); #1;
      if (s < STAGES-1) check(dout === 1'b0, "not yet propagated");
    end
    check(dout === 1'b1, "1 seen after STAGES clocks");

    // falling level -> same latency
    phase = "FALL"; @(negedge dst_clk); din = 1'b0;
    for (s = 0; s < STAGES; s = s + 1) @(posedge dst_clk);
    #1; check(dout === 1'b0, "0 seen after STAGES clocks");
